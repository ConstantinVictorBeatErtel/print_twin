...
**User capture update, September 5:** the target room should contain no people. Prefer an unobstructed standing viewpoint beside the demo placement table; the room center is useful when it gives a better view of that area. Preserve furniture and architecture during any image cleanup, and mark occluded surfaces filled by generation as inferred. The current room still contains people. The user has paused new world generation while preparing a cleaner capture. The implemented pipeline, capture instructions, and planned cleanup process are recorded in [Room creation workflow](docs/ROOM_CREATION_WORKFLOW.md).
